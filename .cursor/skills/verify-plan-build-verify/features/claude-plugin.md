...
The Claude tree is `plugins/claude`. A Claude Code install surfaces skills `plan`, `build`, and `verify`, agents `plan-agent`, `build-agent`, and `verify-agent`, and commands `/plan`, `/build`, and `/verify`. CI proves the tree with `claude plugin validate ./plugins/claude --strict`.
...
- **Skills and agents.** `evidence/claude-plugin/listing.txt` lists skills `build, plan, ship, triage, verify` and agents `build-agent.md, plan-agent.md, verify-agent.md`.
...
- Live Claude plugin UI is issue #3. Record it as `verified-unreachable` without a dedicated Claude session.
