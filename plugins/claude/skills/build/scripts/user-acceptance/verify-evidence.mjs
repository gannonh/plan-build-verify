...
function isLinearIssueUrl(value) {
  try {
    const url = new URL(value);
    return url.protocol === 'https:' && url.hostname === 'linear.app' && /\/issue\/[A-Z0-9]+-\d+/i.test(url.pathname);
  } catch {
    return false;
  }
}
...
if (manifest) {
  ...
  if (manifest.mode === 'technical-enablement') {
    ...
    if (exception?.approval_ref && !isLinearIssueUrl(exception.approval_ref)) {
      fail('technical_enablement approval_ref must be a Linear issue URL');
    }
    if (exception?.dependency_ref && !isLinearIssueUrl(exception.dependency_ref)) {
      fail('technical_enablement dependency_ref must be a Linear issue URL');
    }
  }
