...
2. **Evidence before recommendation**
   - Do not treat summaries, static code review, or green unit tests as AC evidence.
...
5. **Video is ideal, not an unbounded gate**
   ...
   - Do not install or debug a new recording stack solely for ideal video during this workflow. Missing video alone does not fail or block a slice unless an acceptance criterion explicitly requires temporal proof.
...
7. **Evidence is not merge permission**
   - Record the matrix in the PR body and Linear comments as the owning skill requires.
   - Merge permission is the Merging state. This workflow does not merge.

8. **Do not claim a criterion passed without artifacts**
   - Evidence collection supports the matrix.
   - Leave `Fail` and `Blocked` rows unchecked in wording and in the matrix.
...
Use the bundled helpers when practical. Resolve script paths relative to the `build` skill directory (`scripts/user-acceptance/`). Verify reuses these same files from the sibling `build` skill against merged code.
...
```json
{
  "approval_ref": "https://linear.app/<workspace>/issue/<id>",
  ...
  "dependency_ref": "Linear issue URL for the native dependency"
}
```
...
If a Linear issue or PR is known, offer or perform a status update with:
...
Build records this in the PR body and a Linear comment starting with `## Build: acceptance criteria matrix`. Verify records merged-state results in a Linear comment starting with `## Verify: acceptance criteria matrix`. Merge permission is the Merging state, not this evidence report.
...
- Declaring merge readiness from this evidence workflow.
